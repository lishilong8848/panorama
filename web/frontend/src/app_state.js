...
function normalizeInternalDownloadPoolSlot(slot) {
  ...
  function normalizeAlarmEventReadinessOverview({ key, title, payload }) {
    ...
    if (uploadRunning) {
      tone = "info";
      statusText = "正在上传";
      summaryText = uploadRunningText || "外网正在上传告警信息文件。";
    } else if (failedBuildings.length) {
      tone = "danger";
      statusText = "存在失败楼栋";
    ...
    if (uploadLastRunAt) {
      ...
      if (uploadLastError) {
        summaryText += ` 最近上传异常：${uploadLastError}`;
      }
    }
    return {
      key,
    ...
    const map = {
      ...
      alarm_event_upload: (() => {
        ...
        return {
          eyebrow: "专项上传",
          title: "告警信息上传",
          description: "读取 08 点和 16 点共享告警文件，筛选 60 天内记录并写入目标多维表。",
          metrics: [
