...
def _repair_handover_segment_required_defaults(config_path: str | Path) -> bool:
    changed = False
    for building in HANDOVER_SEGMENT_BUILDINGS:
        ...
        if str(sheet_names.get(building, "") or "").strip():
            continue
...
def _ensure_handover_segment_files(cfg: Dict[str, Any], config_path: str | Path | None = None) -> bool:
    ...
    if has_all_handover_segment_files(target):
        return _repair_handover_segment_required_defaults(target)
    with handover_segment_write_lock():
        ...
        _repair_handover_segment_required_defaults(target)
    return True
