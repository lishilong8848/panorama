...
def _refresh_runtime_status_after_task_mutation(container, reason: str) -> None:
    coordinator = getattr(container, "runtime_status_coordinator", None)
    ...
    try:
        ...
        try:
            container.add_system_log(f"[任务面板] 取消后刷新运行状态快照失败: {exc}")
        except Exception:
            pass
    try:
...
def _collect_indexed_branch_source_units(
    ...
    *,
    ...
    """Collect daily branch source units from the bridge index only."""
    ...
    for business_date in [str(item or "").strip() for item in bucket_keys if str(item or "").strip()]:
        ...
        bucket_source_units[business_date] = units
        missing_by_bucket[business_date] = missing
    ready_bucket_keys = [
        bucket_key
...
def _build_review_links_for_base_url(review_cfg: Dict[str, Any], effective_base_url: str) -> list[Dict[str, str]]:
    ...
    if not any(str(item.get("code", "")).strip().lower() == "h" for item in links):
        links.append(
            {
                "building": "H楼",
                "code": "h",
                "url": f"{effective_base_url}/handover/review/h",
            }
        )
    return links
...
def job_chiller_mode_upload_run(request: Request) -> Dict[str, Any]:
    ...
    try:
        job = _start_background_job(
            ...
            worker_handler="",
            worker_payload={},
            resource_keys=_job_resource_keys("network:external", "shared_bridge:chiller_mode_upload"),
            priority="manual",
...
def job_branch_power_from_download(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    try:
        job = _start_background_job(
            ...
            priority=priority,
            feature="branch_power_external_dispatch_daily",
            dedupe_key=dedupe_key,
            submitted_by=submitted_by,
        )
        container.add_system_log(
    ...
    if bridge_task_id and wait_reason == "waiting:shared_bridge":
        ...
        if bridge_service is not None:
            try:
                ...
                try:
                    container.add_system_log(f"[任务] 等待任务已取消，但绑定补采任务取消失败 job_id={job_id}, bridge_task_id={bridge_task_id}, error={exc}")
                except Exception:
                    pass
    _refresh_runtime_status_after_task_mutation(container, "job_cancel")
    return {
        "ok": True,
        "accepted": True,
