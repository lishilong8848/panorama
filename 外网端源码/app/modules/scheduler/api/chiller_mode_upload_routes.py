...
router = APIRouter(prefix="/api/scheduler/chiller-mode-upload", tags=["scheduler-chiller-mode-upload"])
...
def _scheduler_cfg_from_v3(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(features, dict):
        return {}
    upload_cfg = features.get("chiller_mode_upload", {})
...
def _build_payload(container, action_result: Dict[str, Any] | None = None) -> Dict[str, Any]:
    snapshot = container.chiller_mode_upload_scheduler_status()
    return {
        "ok": True,
        ...
        "executor_bound": bool(container.is_chiller_mode_upload_scheduler_executor_bound()),
        "callback_name": container.chiller_mode_upload_scheduler_executor_name(),
...
@router.post("/start")
def chiller_mode_upload_scheduler_start(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(container, path=("features", "chiller_mode_upload", "scheduler"), auto_start_in_gui=True)
    action = container.start_chiller_mode_upload_scheduler()
    return _build_payload(container, action_result=action)


@router.post("/stop")
def chiller_mode_upload_scheduler_stop(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(container, path=("features", "chiller_mode_upload", "scheduler"), auto_start_in_gui=False)
    action = container.stop_chiller_mode_upload_scheduler()
    return _build_payload(container, action_result=action)


@router.get("/status")
def chiller_mode_upload_scheduler_status(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    return _build_payload(container)


@router.post("/config")
def chiller_mode_upload_scheduler_config(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if not isinstance(upload_cfg, dict):
        upload_cfg = {}
        features["chiller_mode_upload"] = upload_cfg
    scheduler_cfg = upload_cfg.get("scheduler")
    ...
    record_scheduler_config_autostart(
        container,
        path=("features", "chiller_mode_upload", "scheduler"),
        scheduler_cfg=new_cfg,
    )
    data = _build_payload(container)
    data.update(
        {
            "message": "制冷模式参数上传调度配置已更新并热重载",
