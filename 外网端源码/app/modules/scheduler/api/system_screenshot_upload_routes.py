...
router = APIRouter(
    prefix="/api/scheduler/system-screenshot-upload",
    tags=["scheduler-system-screenshot-upload"],
)
...
def _scheduler_cfg_from_v3(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(features, dict):
        return {}
    upload_cfg = features.get("system_screenshot_upload", {})
...
def _build_payload(container, action_result: Dict[str, Any] | None = None) -> Dict[str, Any]:
    snapshot = container.system_screenshot_upload_scheduler_status()
    ...
    return {
        ...
        "executor_bound": bool(container.is_system_screenshot_upload_scheduler_executor_bound()),
        "callback_name": container.system_screenshot_upload_scheduler_executor_name(),
...
@router.get("/status")
def system_screenshot_upload_scheduler_status(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    return _build_payload(container)


@router.post("/config")
def system_screenshot_upload_scheduler_config(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    if not isinstance(upload_cfg, dict):
        upload_cfg = {}
        features["system_screenshot_upload"] = upload_cfg
    scheduler_cfg = upload_cfg.get("scheduler")
    ...
    try:
        persist_full_config(container, merged, source="系统截图上传调度配置保存", mode="light")
    except Exception as exc:
