...
router = APIRouter(
    prefix="/api/scheduler/temperature-humidity-upload",
    tags=["scheduler-temperature-humidity-upload"],
)
...
def _scheduler_cfg_from_v3(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(features, dict):
        return {}
    upload_cfg = features.get("temperature_humidity_upload", {})
...
def _build_payload(container, action_result: Dict[str, Any] | None = None) -> Dict[str, Any]:
    snapshot = container.temperature_humidity_upload_scheduler_status()
    scheduler_cfg = _scheduler_cfg_from_v3(container.config)
    return {
        "ok": True,
        ...
        "run_time": str(scheduler_cfg.get("run_time") or snapshot.get("run_time") or "").strip(),
        ...
        "executor_bound": bool(container.is_temperature_humidity_upload_scheduler_executor_bound()),
        "callback_name": container.temperature_humidity_upload_scheduler_executor_name(),
...
@router.post("/start")
def temperature_humidity_upload_scheduler_start(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(
        container,
        path=("features", "temperature_humidity_upload", "scheduler"),
        auto_start_in_gui=True,
    )
    action: Dict[str, Any] | None = None
    reloader = getattr(container, "rebuild_temperature_humidity_upload_scheduler", None)
    if callable(reloader):
        reload_result = reloader(source="调度启动")
        if isinstance(reload_result, dict) and isinstance(reload_result.get("action"), dict):
            action = reload_result["action"]
    if action is None:
        action = container.start_temperature_humidity_upload_scheduler()
    return _build_payload(container, action_result=action)


@router.post("/stop")
def temperature_humidity_upload_scheduler_stop(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(
        container,
        path=("features", "temperature_humidity_upload", "scheduler"),
    ...
    action = container.stop_temperature_humidity_upload_scheduler()
    return _build_payload(container, action_result=action)


@router.get("/status")
def temperature_humidity_upload_scheduler_status(request: Request) -> Dict[str, Any]:
    return _build_payload(request.app.state.container)


@router.post("/config")
def temperature_humidity_upload_scheduler_config(
    ...
    if not isinstance(upload_cfg, dict):
        upload_cfg = {}
        features["temperature_humidity_upload"] = upload_cfg
    scheduler_cfg = upload_cfg.get("scheduler")
    ...
    for key in ALLOWED_KEYS:
        ...
        if key in {"enabled", "auto_start_in_gui", "catch_up_if_missed", "retry_failed_in_same_period"}:
    ...
    try:
        persist_full_config(container, merged, source="空调温湿度上传调度配置保存", mode="light")
    except Exception as exc:
    ...
    scheduler = getattr(container, "temperature_humidity_upload_scheduler", None)
    if run_time_changed and scheduler is not None:
        reset_result = scheduler.reset_today_state_for_run_time_change()
