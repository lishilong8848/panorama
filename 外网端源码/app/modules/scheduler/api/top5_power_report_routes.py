...
router = APIRouter(prefix="/api/scheduler/top5-power-report", tags=["scheduler-top5-power-report"])
...
def _scheduler_cfg_from_v3(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(handover, dict):
        return {}
    top5_cfg = handover.get("top5_power_report", {})
...
def _build_payload(container, action_result: Dict[str, Any] | None = None) -> Dict[str, Any]:
    snapshot = container.top5_power_report_scheduler_status()
    return {
        "ok": True,
        ...
        "executor_bound": bool(container.is_top5_power_report_scheduler_executor_bound()),
        "callback_name": container.top5_power_report_scheduler_executor_name(),
...
@router.post("/start")
def top5_power_report_scheduler_start(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(
        container,
        path=("features", "handover_log", "top5_power_report", "scheduler"),
        auto_start_in_gui=True,
    )
    action = container.start_top5_power_report_scheduler()
    return _build_payload(container, action_result=action)


@router.post("/stop")
def top5_power_report_scheduler_stop(request: Request) -> Dict[str, Any]:
    container = request.app.state.container
    persist_scheduler_toggle(
        container,
        path=("features", "handover_log", "top5_power_report", "scheduler"),
    ...
    action = container.stop_top5_power_report_scheduler()
    return _build_payload(container, action_result=action)


@router.get("/status")
def top5_power_report_scheduler_status(request: Request) -> Dict[str, Any]:
    return _build_payload(request.app.state.container)


@router.post("/config")
def top5_power_report_scheduler_config(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    # TOP5 only runs at the configured monthly slot. Starting the app must never
    # replay a missed slot and unexpectedly generate another report.
    scheduler_patch["catch_up_if_missed"] = False
    ...
    try:
        patch_result = save_handover_common_scheduler_patch(
            ...
            source="TOP5功率文件生成调度配置保存",
        )
        container.reload_config(patch_result["saved_config"])
    except Exception as exc:
    ...
    if schedule_changed and container.top5_power_report_scheduler:
        reset_result = container.top5_power_report_scheduler.reset_current_month_state_for_schedule_change()

    status_payload = _build_payload(container)
    data = dict(status_payload)
    data.update(
        {
            "message": "TOP5功率文件生成调度配置已更新并热重载",
            "schedule_changed": schedule_changed,
            "state_reset": reset_result,
            "scheduler_status": status_payload,
            "scheduler_config": {
                **{key: new_cfg.get(key) for key in sorted(ALLOWED_KEYS)},
