...
_SOURCE_SNAPSHOT_SCOPE_CODE_FRONTEND_STATIC = "code_frontend_static"
_SOURCE_FRONTEND_DIST_PREFIXES = (
    "web/frontend/dist/",
    "web_frontend/dist/",
)
...
def _is_frontend_dist_static_relpath(rel_path: Path | str) -> bool:
    rel = Path(str(rel_path).replace("\\", "/"))
    rel_text = _normalize_zip_relpath(rel)
    if not rel_text or rel.name in {"", ".", ".."}:
        return False
    if rel.name in _SOURCE_SNAPSHOT_EXCLUDED_FILES:
        return False
    return any(rel_text.startswith(prefix) for prefix in _SOURCE_FRONTEND_DIST_PREFIXES)
...
class UpdaterService:
    ...
    def _git_tracked_source_snapshot_files(self) -> List[Path]:
        ...
        for raw in (ret.stdout or "").splitlines():
            ...
            rel_text = _normalize_zip_relpath(rel)
            if rel_text in seen or not _is_source_snapshot_relpath(rel):
                continue
    ...
    def _build_source_snapshot_zip(self, *, manifest: Dict[str, Any], zip_path: Path) -> Dict[str, Any]:
        ...
        if zip_path.exists():
            zip_path.unlink()
        source_files = self._git_tracked_source_snapshot_files()
        if not source_files:
            raise RuntimeError("当前 Git 工作区未找到可同步的代码或前端静态资源文件。")
        ...
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
            ...
            embedded_manifest["scope"] = _SOURCE_SNAPSHOT_SCOPE_CODE_FRONTEND_STATIC
        ...
        return {
            ...
            "files": file_entries,
            "scope": _SOURCE_SNAPSHOT_SCOPE_CODE_FRONTEND_STATIC,
        }
    ...
    def _auto_publish_git_head_to_internal(self) -> Dict[str, Any]:
        ...
        self._log(
            "外网端检测到 Git HEAD 变化，已发布代码和前端静态资源同步包并下发内网应用命令: "
