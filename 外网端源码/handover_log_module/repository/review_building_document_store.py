...
class ReviewBuildingDocumentStore:
    ...
    def _create_schema(conn: sqlite3.Connection) -> None:
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_xlsx_write_jobs_status
                ON xlsx_write_jobs(status, created_at);
        ...
        xlsx_columns = {
            str(row["name"] or "").strip().lower()
            for row in conn.execute("PRAGMA table_info(xlsx_write_jobs)").fetchall()
        }
        if "session_id" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN session_id TEXT NOT NULL DEFAULT ''")
        if "dedupe_key" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN dedupe_key TEXT NOT NULL DEFAULT ''")
        if "payload_json" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN payload_json TEXT NOT NULL DEFAULT '{}'")
        if "attempts" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN attempts INTEGER NOT NULL DEFAULT 0")
        if "error" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN error TEXT NOT NULL DEFAULT ''")
        if "finished_at" not in xlsx_columns:
            conn.execute("ALTER TABLE xlsx_write_jobs ADD COLUMN finished_at TEXT NOT NULL DEFAULT ''")
    ...
    @staticmethod
    def _row_to_xlsx_job(row: sqlite3.Row | None) -> Dict[str, Any] | None:
        if row is None:
            return None
        return {
    ...
    def enqueue_xlsx_write_job(
        ...
        *,
        ...
        if task_type_text not in {"review_excel_sync", "capacity_overlay_sync", "barrier"}:
            raise ValueError(f"unsupported xlsx write task_type: {task_type}")
        session_id_text = str(session_id or "").strip()
        dedupe_key_text = str(dedupe_key or "").strip()
        payload_text = _json_dumps(payload if isinstance(payload, dict) else {})
        now = _now_text()
        with self.connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            if task_type_text != "barrier" and dedupe_key_text:
                pending = conn.execute(
                    ...
                    SELECT *
                     ...
                     LIMIT 1
                    """,
                    (task_type_text, dedupe_key_text),
                ).fetchone()
                if pending is not None:
                    conn.execute(
                        ...
                        UPDATE xlsx_write_jobs
                         ...
                         WHERE job_id=?
                        """,
                        (session_id_text, payload_text, now, str(pending["job_id"] or "")),
                    )
                    row = conn.execute(
            ...
            conn.execute(
                ...
                INSERT INTO xlsx_write_jobs(
                    job_id, building, session_id, task_type, dedupe_key,
                    payload_json, status, attempts, error, created_at, updated_at, finished_at
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    job_id,
    ...
    def claim_next_xlsx_write_job(self) -> Dict[str, Any] | None:
        ...
        with self.connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            running = conn.execute(
                "SELECT job_id FROM xlsx_write_jobs WHERE status='running' LIMIT 1"
            ).fetchone()
            if running is not None:
                return None
            row = conn.execute(
            ...
            conn.execute(
                ...
                UPDATE xlsx_write_jobs
                 ...
                 WHERE job_id=?
                   AND status='pending'
                   AND NOT EXISTS (
                       SELECT 1 FROM xlsx_write_jobs WHERE status='running'
                   )
                """,
                (now, job_id),
            )
            claimed = conn.execute(
                "SELECT * FROM xlsx_write_jobs WHERE job_id=? AND status='running'",
                (job_id,),
            ).fetchone()
            return self._row_to_xlsx_job(claimed)
        ...
        with self.connect() as conn:
            ...
            conn.execute(
                ...
                UPDATE xlsx_write_jobs
                 ...
                 WHERE job_id=?
                """,
                (status, str(error or "").strip(), now, now, job_id_text),
            )
            row = conn.execute("SELECT * FROM xlsx_write_jobs WHERE job_id=?", (job_id_text,)).fetchone()
            return self._row_to_xlsx_job(row)

    def recover_xlsx_write_jobs_for_startup(self) -> int:
        self.ensure_ready()
        now = _now_text()
        with self.connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
