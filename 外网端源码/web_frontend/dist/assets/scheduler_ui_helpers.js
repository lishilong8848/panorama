export function createSchedulerUiHelpers(options = {}) {
  ...
  function getSchedulerSnapshotByKey(key) {
    ...
    if (normalized === "alarm_rule_export_upload") return health.alarm_rule_export_upload?.scheduler || {};
  ...
  function syncSchedulerDraftAutoStartFromRemembered(key, rememberedEnabled) {
    ...
    if (
      normalized === "branch_power_upload"
      && config?.value?.branch_power_upload?.scheduler
      && typeof config.value.branch_power_upload.scheduler === "object"
    ) {
      config.value.branch_power_upload.scheduler.auto_start_in_gui = remembered;
      return;
    }
    if (
    ...
    if (
      normalized === "system_screenshot_upload"
    ...
    if (
      normalized === "alarm_rule_export_upload"
      && config?.value?.alarm_rule_export_upload?.scheduler
      && typeof config.value.alarm_rule_export_upload.scheduler === "object"
    ) {
      config.value.alarm_rule_export_upload.scheduler.auto_start_in_gui = remembered;
      config.value.alarm_rule_export_upload.scheduler.catch_up_if_missed = false;
      return;
    }
    if (
