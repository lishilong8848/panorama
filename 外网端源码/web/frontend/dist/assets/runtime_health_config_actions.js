...
export function createRuntimeHealthConfigActions(ctx) {
  ...
  function buildAutomaticUpdaterOverlayPayload(runtime, options = {}) {
    ...
    if (isSharedMirrorUpdater(runtime)) {
      return {
        title: "已检测到外网新版本",
        subtitle: "程序正在自动更新，请保持当前页面打开，完成后会自动恢复。",
        stage: "applying",
        kicker: "自动跟随更新",
      };
    }
  ...
  function handleUpdaterRuntimeSideEffects(_previousRuntime, nextRuntime) {
  ...
  function applyHealthSnapshot(data) {
    ...
    if (data.branch_power_upload && typeof data.branch_power_upload === "object") {
      if (data.branch_power_upload.scheduler && typeof data.branch_power_upload.scheduler === "object") {
        Object.assign(health.branch_power_upload.scheduler, {
          ...
          minute_offset: Number.parseInt(String(data.branch_power_upload.scheduler.minute_offset ?? 240), 10) || 0,
    ...
    if (data.shared_bridge && typeof data.shared_bridge === "object") {
      ...
      if (String(health?.deployment?.role_mode || "").trim().toLowerCase() !== "internal") {
        ...
        delete nextSharedBridge.internal_source_cache;
        delete nextSharedBridge.internal_alert_status;
      }
  ...
  function normalizeHandoverBuildingName(building) {
    ...
    if (["A楼", "B楼", "C楼", "D楼", "E楼", "H楼"].includes(raw)) return raw;
    const upper = raw.toUpperCase();
    if (["A", "B", "C", "D", "E", "H"].includes(upper)) return `${upper}楼`;
    return "A楼";
  }
