...
FAMILY_DIR_NAMES = {
    ...
    FAMILY_TOP5_MONTHLY_REPORT: "top5_monthly_report",
...
class SharedSourceCacheService:
    ...
    def __init__(
        ...
        *,
        ...
        self._family_status: Dict[str, Dict[str, Any]] = {
            ...
            FAMILY_TOP5_MONTHLY_REPORT: {"ready_count": 0, "failed_buildings": [], "blocked_buildings": [], "last_success_at": ""},
    ...
    def _build_external_full_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            snapshot_error = snapshot_error or str(exc)
        try:
            top5_monthly_family = self._build_family_health_snapshot(
                source_family=FAMILY_TOP5_MONTHLY_REPORT,
                current_bucket=current_bucket,
                include_latest_selection=include_latest_selection,
            )
        except Exception as exc:  # noqa: BLE001
            top5_monthly_family = {"current_bucket": current_bucket, "buildings": []}
            snapshot_error = snapshot_error or str(exc)
        try:
        ...
        try:
            ...
            snapshot_error = snapshot_error or str(exc)
        try:
            building_full_cabinet_power_family = self._build_family_health_snapshot(
        ...
        families[FAMILY_TOP5_MONTHLY_REPORT] = {**families.get(FAMILY_TOP5_MONTHLY_REPORT, {}), **top5_monthly_family}
        ...
        families[FAMILY_BUILDING_FULL_CABINET_POWER] = {
        ...
        snapshot = {
            ...
            FAMILY_TOP5_MONTHLY_REPORT: families.get(FAMILY_TOP5_MONTHLY_REPORT, {}),
    ...
    def get_external_source_cache_overview_fast(self) -> Dict[str, Any]:
        ...
        snapshot = {
            ...
            FAMILY_TOP5_MONTHLY_REPORT: self._build_fast_external_family_from_index(
                source_family=FAMILY_TOP5_MONTHLY_REPORT,
                current_bucket=current_bucket,
                buildings=buildings,
            ),
            ...
            FAMILY_BUILDING_FULL_CABINET_POWER: self._build_fast_external_family_from_index(
                source_family=FAMILY_BUILDING_FULL_CABINET_POWER,
    ...
    def _refresh_config(self) -> None:
        ...
        chiller_mode_switch_cfg = (
            ...
            and isinstance(self.runtime_config.get("handover_log", {}).get("chiller_mode_switch", {}), dict)
            else {}
        )
        self._chiller_mode_switch_interval_sec = max(
            60,
            int(chiller_mode_switch_cfg.get("interval_sec", 600) or 600),
        )
        ...
        self._top5_monthly_cache_root = self.shared_root / FAMILY_LABELS[FAMILY_TOP5_MONTHLY_REPORT] if self.shared_root else None
    ...
    def _normalize_source_family(self, source_family: str) -> str:
        ...
        if text in {FAMILY_TOP5_MONTHLY_REPORT, "top5_monthly_report"}:
            return FAMILY_TOP5_MONTHLY_REPORT
    ...
    @staticmethod
    def _parse_chiller_mode_switch_bucket(bucket_key: str, *, fallback: datetime | None = None) -> datetime:
        text = str(bucket_key or "").strip().replace("/", "-")
    ...
    def get_health_snapshot(self, *, mode: str = "external_full") -> Dict[str, Any]:
        ...
        if normalized_mode == "internal_light":
            ...
            chiller_mode_switch_family = self._build_internal_light_family_snapshot(
                source_family=FAMILY_CHILLER_MODE_SWITCH,
                current_bucket=chiller_mode_switch_bucket,
                cached_rows=light_building_status.get(FAMILY_CHILLER_MODE_SWITCH, {}),
                active_downloads=active_downloads,
            )
    ...
    def _ensure_dirs(self) -> None:
        ...
        if self._top5_monthly_cache_root is not None:
            self._top5_monthly_cache_root.mkdir(parents=True, exist_ok=True)
    ...
    def get_required_directory_paths(self) -> Dict[str, str]:
        return {
            ...
            FAMILY_TOP5_MONTHLY_REPORT: str(self._top5_monthly_cache_root) if self._top5_monthly_cache_root is not None else "",
    ...
    def _family_root(self, source_family: str) -> Path:
        ...
        if normalized_family == FAMILY_TOP5_MONTHLY_REPORT and self._top5_monthly_cache_root is not None:
            return self._top5_monthly_cache_root
        ...
        if (
            normalized_family == FAMILY_BUILDING_FULL_CABINET_POWER
    ...
    def _chiller_mode_switch_temp_root(self, *, bucket_key: str, building: str) -> Path:
        if self._tmp_root is None:
            raise RuntimeError("共享缓存临时目录未配置")
        bucket_segment = re.sub(r"[^\d]+", "", str(bucket_key or "").strip())[:12] or datetime.now().strftime("%Y%m%d%H%M")
        return self._tmp_root / "chiller_mode_switch_latest" / bucket_segment / building
    ...
    def _ensure_internal_source_writer(self, action: str) -> None:
        if self.role_mode != "internal":
            raise RuntimeError(f"{action}仅允许内网端执行，外网端只读取已登记索引")
    ...
    def _store_alias_entry(
        ...
        *,
        ...
        duty_date: str,
        duty_shift: str,
        origin_entry: Dict[str, Any],
        metadata: Dict[str, Any] | None = None,
    ) -> Dict[str, Any]:
        self._ensure_internal_source_writer("源文件索引别名登记")
        if self.store is None:
            raise RuntimeError("共享缓存存储未初始化")
        normalized_family = self._normalize_source_family(source_family)
        ...
        size_bytes = int(origin_entry.get("size_bytes", 0) or 0)
        if size_bytes <= 0:
            try:
    ...
    def _resolve_entry_file_path(self, entry: Dict[str, Any] | None) -> Path | None:
        ...
        if not _is_accessible_cached_file(file_path):
            ...
            return None
        if self.role_mode == "internal" and normalized_family == FAMILY_HANDOVER_LOG:
            try:
    ...
    def _get_latest_ready_handover_entry_for_date(
        ...
        *,
        ...
        if isinstance(cached_entry, dict):
            ...
            metadata = normalized_entry.get("metadata", {}) if isinstance(normalized_entry.get("metadata", {}), dict) else {}
            resolution_source = str(metadata.get("resolution_source", "") or "").strip()
            if not resolution_source:
                if bool(metadata.get("alias_only", False)):
                    resolution_source = "cache_alias"
                elif str(normalized_entry.get("bucket_kind", "") or "").strip().lower() == "latest":
                    resolution_source = "cache_latest"
                else:
                    resolution_source = "cache_date"
            normalized_entry["resolution_source"] = resolution_source
            normalized_entry["scanned_candidates"] = []
            normalized_entry["redownload_attempted"] = False
            return normalized_entry
        if callable(emit_log):
            emit_log(
                f"[12项历史共享文件补采] 未找到内网端已登记的交接班源文件索引: "
    ...
    def _get_date_semantic_entries(self, *, source_family: str, selected_dates: List[str], buildings: List[str] | None = None) -> List[Dict[str, Any]]:
        if self.store is None:
            return []
        self._repair_stale_refreshing_entries()
        requested = [str(item or "").strip() for item in (buildings or self.get_enabled_buildings()) if str(item or "").strip()]
    ...
    def get_monthly_by_date_entries(self, *, selected_dates: List[str], buildings: List[str] | None = None) -> List[Dict[str, Any]]:
        return self._get_date_semantic_entries(
            source_family=FAMILY_MONTHLY_REPORT,
            selected_dates=selected_dates,
            buildings=buildings,
        )

    def get_top5_monthly_by_date_entries(self, *, selected_dates: List[str], buildings: List[str] | None = None) -> List[Dict[str, Any]]:
        return self._get_date_semantic_entries(
            source_family=FAMILY_TOP5_MONTHLY_REPORT,
            selected_dates=selected_dates,
            buildings=buildings,
    ...
    @staticmethod
    def _parse_branch_business_date(value: str, *, fallback: datetime | None = None) -> datetime:
        raw = str(value or "").strip().replace("/", "-")
        for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d %H", "%Y-%m-%d", "%Y%m%d%H", "%Y%m%d"):
            try:
        ...
        if len(digits) >= 8:
            try:
                return datetime.strptime(digits[:8], "%Y%m%d").replace(hour=0, minute=0, second=0, microsecond=0)
            except ValueError:
                pass
        return ((fallback or datetime.now()) - timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    def branch_power_day_bucket_keys(self, business_date: str) -> List[str]:
        day_dt = self._parse_branch_business_date(business_date)
        return [day_dt.replace(hour=hour).strftime("%Y-%m-%d %H") for hour in range(24)]

    def building_full_cabinet_power_day_bucket(self, when: datetime | None = None) -> str:
        return self.branch_power_business_date(when)
    ...
    def fill_chiller_mode_switch_latest(
        self,
        *,
        building: str,
        bucket_key: str,
        ...
        self._ensure_internal_source_writer("制冷单元模式切换参数源文件补采")
        resolved_bucket = str(bucket_key or "").strip() or self.current_chiller_mode_switch_bucket()
        start_time, end_time = self._chiller_mode_switch_query_window(resolved_bucket)
        temp_root = self._chiller_mode_switch_temp_root(bucket_key=resolved_bucket, building=building)
        cfg = load_handover_config(self.runtime_config)
        ...
        emit_log(
            "[制冷单元模式切换参数源文件] 开始 "
            f"building={building}, bucket={resolved_bucket}, query={start_time}~{end_time}, scale=5分钟"
        )
        result = service.run_chiller_mode_switch_only(
            buildings=[building],
        ...
        if not success_files:
            raise RuntimeError(f"制冷单元模式切换参数源文件下载失败: {building}")
        item = success_files[0]
        source_path = Path(str(item.get("file_path", "") or "").strip())
        if not source_path.exists():
            raise FileNotFoundError(f"下载完成后的制冷单元模式切换参数源文件不存在: {source_path}")
        return self._store_entry(
            source_family=FAMILY_CHILLER_MODE_SWITCH,
            building=building,
            bucket_kind="latest",
    ...
    def _fill_branch_daily_latest(
        ...
        *,
        ...
        if normalized_family not in {
            ...
            raise RuntimeError(f"不支持的支路整日源文件类型: {source_family}")
        self._ensure_internal_source_writer(f"{FAMILY_LABELS.get(normalized_family, normalized_family)}整日源文件补采")
        day_dt = self._parse_branch_business_date(business_date)
        resolved_business_date = day_dt.strftime("%Y-%m-%d")
        start_time, end_time, covered_buckets = self._branch_day_query_window(resolved_business_date)
        if self._tmp_root is None:
            raise RuntimeError("共享缓存临时目录未配置")
        family_temp_names = {
            FAMILY_BRANCH_POWER: "branch_power_daily",
            FAMILY_BRANCH_CURRENT: "branch_current_daily",
            FAMILY_BRANCH_SWITCH: "branch_switch_daily",
            FAMILY_BUILDING_FULL_CABINET_POWER: "building_full_cabinet_power_daily",
        }
        runner_names = {
            FAMILY_BRANCH_POWER: "run_branch_power_only",
            FAMILY_BRANCH_CURRENT: "run_branch_current_only",
            FAMILY_BRANCH_SWITCH: "run_branch_switch_only",
        ...
        emit_log(
            f"[{family_label}整日源文件补采] 开始 building={building}, business_date={resolved_business_date}, "
            f"query={start_time}~{end_time}"
        )
        runner = getattr(service, runner_names[normalized_family])
        result = runner(
            buildings=[building],
            start_time=start_time,
            end_time=end_time,
    ...
    def fill_branch_power_day_latest(
        ...
        *,
        ...
        return self._fill_branch_daily_latest(
            source_family=FAMILY_BRANCH_POWER,
            building=building,
            business_date=business_date or bucket_key,
            emit_log=emit_log,
        )

    def fill_branch_current_day_latest(
        self,
        *,
        building: str,
        business_date: str = "",
    ...
    def fill_branch_switch_day_latest(
        self,
        *,
        building: str,
        business_date: str = "",
    ...
    def fill_building_full_cabinet_power_day_latest(
        self,
        *,
        building: str,
        business_date: str = "",
        ...
        resolved_business_date = self._parse_branch_business_date(business_date or bucket_key).strftime("%Y-%m-%d")
        ...
        result = service.run_building_full_cabinet_power_only(
            buildings=[building],
            start_time=start_time,
            end_time=end_time,
            switch_network=False,
    ...
    def fill_handover_capacity_latest(self, *, building: str, bucket_key: str, emit_log: Callable[[str], None]) -> Dict[str, Any]:
        self._ensure_internal_source_writer("交接班容量最新源文件补采")
        cfg = load_handover_config(self.runtime_config)
    ...
    def fill_handover_history(self, *, buildings: List[str], duty_date: str, duty_shift: str, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        self._ensure_internal_source_writer("交接班历史源文件补采")
        normalized_shift = str(duty_shift or "").strip().lower()
        if normalized_shift not in {"day", "night"}:
            raise RuntimeError("交接班历史补采必须指定 day/night 班次，禁止生成通用交接班目录")
        cfg = load_handover_config(self.runtime_config)
        temp_root = self._handover_temp_root(
            bucket_kind="date",
            bucket_key=duty_date,
            duty_date=duty_date,
            duty_shift=normalized_shift,
        )
        service = HandoverDownloadService(
            cfg,
            download_browser_pool=self.download_browser_pool,
        ...
        result = service.run(buildings=buildings, duty_date=duty_date, duty_shift=normalized_shift, switch_network=False, reuse_cached=False, emit_log=emit_log)
        success_files = result.get("success_files", []) if isinstance(result.get("success_files", []), list) else []
        output: List[Dict[str, Any]] = []
        for item in success_files:
            ...
            output.append(
                self._store_entry(
                    ...
                    source_path=source_path,
                    status="ready",
                    metadata={"family": FAMILY_HANDOVER_LOG, "building": building, "duty_date": duty_date, "duty_shift": normalized_shift},
                )
            )
        return output
    ...
    def fill_handover_capacity_history(self, *, buildings: List[str], duty_date: str, duty_shift: str, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        self._ensure_internal_source_writer("交接班容量历史源文件补采")
        normalized_shift = str(duty_shift or "").strip().lower()
        if normalized_shift not in {"day", "night"}:
            raise RuntimeError("交接班容量历史补采必须指定 day/night 班次，禁止生成通用交接班目录")
        cfg = load_handover_config(self.runtime_config)
        temp_root = self._handover_capacity_temp_root(
            bucket_kind="date",
            bucket_key=duty_date,
    ...
    def fill_day_metric_history(self, *, selected_dates: List[str], building_scope: str, building: str | None, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        self._ensure_internal_source_writer("12项源文件索引别名登记")
        target_buildings = (
            [str(building or "").strip()]
        ...
        for duty_date in [str(item or "").strip() for item in (selected_dates or []) if str(item or "").strip()]:
            for building_name in target_buildings:
                ...
                alias_entry = self._store_alias_entry(
                    source_family=FAMILY_HANDOVER_LOG,
                    building=building_name,
                    bucket_kind="date",
                    bucket_key=duty_date,
                    ...
                    metadata={
                        ...
                        "duty_date": duty_date,
                        "duty_shift": "all",
                        "resolution_source": resolution_source or "cache_alias",
                        "source_bucket_kind": cache_bucket_kind,
                        "source_bucket_key": cache_bucket_key,
                        "source_entry_id": cache_entry_id,
                        "selected_source_file": source_file,
                    },
                )
                emit_log(
                    f"[12项历史共享文件补采] 已登记交接班源文件索引别名: duty_date={duty_date}, "
                    f"building={building_name}, source={source_file}, "
                    f"resolution={resolution_source or '-'}, cache={cache_bucket_kind}/{cache_bucket_key}, "
                    f"origin_entry_id={cache_entry_id or '-'}, alias_entry_id={alias_entry.get('entry_id', '') or '-'}"
                )
                output.append(alias_entry)
        if missing_units:
    ...
    def fill_monthly_latest(self, *, building: str, bucket_key: str, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        self._ensure_internal_source_writer("月报最新源文件补采")
        if self._tmp_root is None:
            raise RuntimeError("共享缓存临时目录未配置")
    ...
    def fill_top5_monthly_report_latest(self, *, building: str, bucket_key: str, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        source_family = FAMILY_TOP5_MONTHLY_REPORT
        family_label = FAMILY_LABELS.get(source_family, source_family)
        self._ensure_internal_source_writer(f"{family_label}最新源文件补采")
        if self._tmp_root is None:
            raise RuntimeError("共享缓存临时目录未配置")
        save_dir = self._tmp_root / "top5_monthly_latest" / bucket_key / building
        save_dir.mkdir(parents=True, exist_ok=True)
        cfg = load_handover_config(self.runtime_config)
        service = HandoverDownloadService(
            cfg,
            download_browser_pool=self.download_browser_pool,
            business_root_override=save_dir,
        )
        result = service.run_top5_monthly_report_only(
            buildings=[building],
            switch_network=False,
            reuse_cached=False,
            emit_log=emit_log,
    ...
    def fill_monthly_history(self, *, selected_dates: List[str], buildings: List[str] | None = None, emit_log: Callable[[str], None]) -> List[Dict[str, Any]]:
        self._ensure_internal_source_writer("月报历史源文件补采")
        if self._tmp_root is None:
            raise RuntimeError("共享缓存临时目录未配置")
    ...
    def fill_alarm_event_latest(self, *, building: str, bucket_key: str, emit_log: Callable[[str], None]) -> Dict[str, Any]:
        self._ensure_internal_source_writer("告警最新源文件补采")
    ...
    def fill_alarm_event_manual(self, *, building: str, bucket_key: str, emit_log: Callable[[str], None]) -> Dict[str, Any]:
        self._ensure_internal_source_writer("告警手动源文件补采")
        if self.download_browser_pool is None or not hasattr(self.download_browser_pool, "submit_building_alarm_job"):
            raise RuntimeError("内网下载浏览器池未启动")
    ...
    def _run_chiller_mode_switch_if_due(self, when: datetime | None = None, *, force: bool = False) -> None:
        now_dt = when or datetime.now()
        now_mono = time.monotonic()
        with self._lock:
            elapsed = now_mono - float(self._last_chiller_mode_switch_run_monotonic or 0.0)
        if not force and elapsed < float(self._chiller_mode_switch_interval_sec):
            return
        bucket_key = self.current_chiller_mode_switch_bucket(now_dt)
        self._ensure_dirs()
        self._emit(
    ...
    def _run_current_hour_refresh_impl(self) -> None:
        ...
        self._emit(
            ...
            f"bucket={bucket_key}, 告警bucket={alarm_bucket_key or '-'}"
        )
    ...
    def _resolve_latest_refresh_target(self, *, source_family: str, target_bucket_key: str = "") -> Dict[str, Any]:
        ...
        if normalized_family == FAMILY_BRANCH_POWER:
            return {
                "source_family": normalized_family,
                "bucket_key": self.branch_power_day_bucket(),
                "fill_func": self.fill_branch_power_day_latest,
            }
        if normalized_family == FAMILY_BRANCH_CURRENT:
            return {
                "source_family": normalized_family,
                "bucket_key": self.branch_power_day_bucket(),
                "fill_func": self.fill_branch_current_day_latest,
            }
        if normalized_family == FAMILY_BRANCH_SWITCH:
            return {
                "source_family": normalized_family,
                "bucket_key": self.branch_power_day_bucket(),
                "fill_func": self.fill_branch_switch_day_latest,
            }
        if normalized_family == FAMILY_BUILDING_FULL_CABINET_POWER:
            return {
                "source_family": normalized_family,
        ...
        if normalized_family == FAMILY_CHILLER_MODE_SWITCH:
            return {
                "source_family": normalized_family,
                "bucket_key": self.current_chiller_mode_switch_bucket(),
                "fill_func": self.fill_chiller_mode_switch_latest,
            }
        if normalized_family == FAMILY_ALARM_EVENT:
    ...
    def _run_building_latest_refresh_background(
        ...
        *,
        ...
        try:
            force_download = self._normalize_source_family(source_family) == FAMILY_TOP5_MONTHLY_REPORT
            existing_result = None if force_download else self._existing_latest_refresh_result(
                source_family=source_family,
                building=building,
                bucket_key=bucket_key,
    ...
    def start_building_latest_refresh(
        ...
        *,
        ...
        force_download = normalized_family == FAMILY_TOP5_MONTHLY_REPORT
        existing_result = None if force_download else self._existing_latest_refresh_result(
            source_family=normalized_family,
            building=building_name,
            bucket_key=bucket_key,
            ...
            try:
                ...
                if startup_done:
                    ...
                    if signature != self._last_scheduler_log_signature:
                        ...
                        if (
                            handover_status.get("blocked_buildings")
                            ...
                            self._emit(
                                ...
                                f"制冷模式切换桶={str(chiller_mode_switch_status.get('current_bucket', '') or '').strip() or '-'}"
                            )
            except Exception as exc:  # noqa: BLE001
