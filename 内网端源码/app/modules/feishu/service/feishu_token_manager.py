...
class FeishuTokenManager:
    ...
    def invalidate(self, *, app_id: str, app_secret: str) -> None:
        with self._lock:
            self._entries.pop(self._cache_key(app_id, app_secret), None)
        ...
        if not force_refresh:
            with self._lock:
                ...
                if entry is not None and self._is_entry_valid(entry):
                    return entry.token
        with self._lock:
            refresh_lock = self._refresh_locks.setdefault(key, threading.Lock())
        with refresh_lock:
            if not force_refresh:
                with self._lock:
                    entry = self._entries.get(key)
            ...
            with self._lock:
                self._entries[key] = _TokenEntry(token, time.monotonic() + TOKEN_TTL_SEC)
            return token
