...
def _validate_handover_capacity_report_weather(cfg: Dict[str, Any]) -> None:
    ...
    provider = str(weather.get("provider", "") or "").strip() or "hvac_open_meteo"
    if provider != "hvac_open_meteo":
        raise ValueError("配置错误: features.handover_log.capacity_report.weather.provider 当前仅支持 hvac_open_meteo")
