...
class MonthlyChangeReportService:
    ...
    def _build_output_path(self, *, building: str, target_month: str) -> Path:
        return build_output_base_path(
            ...
            output_type=OUTPUT_TYPE_MONTHLY_CHANGE,
            building=building,
            suffix=".xlsx",
    ...
    def _next_available_output_path(self, *, building: str, target_month: str) -> Path:
        ...
        for idx in range(1, 1000):
            ...
            if not candidate.exists():
                return candidate
        raise RuntimeError(f"月度变更统计表输出文件序号已用尽: {base_path}")
    ...
    def _resolve_existing_output_path(self, *, building: str, target_month: str, output_dir: str) -> Path | None:
        ...
        if not str(output_root).strip():
            return None
        try:
            legacy_pattern, canonical_pattern = monthly_output_patterns(OUTPUT_TYPE_MONTHLY_CHANGE, building)
        except Exception:
            return None
        month_text = "".join(ch for ch in str(target_month or "").strip() if ch.isdigit())[:6]
        bucket_dir = output_root / month_text / f"{month_text}--月度"
