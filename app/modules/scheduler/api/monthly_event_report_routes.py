...
def monthly_event_report_scheduler_config(payload: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    restart_running = bool(container.monthly_event_report_scheduler.is_running()) if container.monthly_event_report_scheduler else False
    try:
        save_scheduler_config_snapshot(
            container,
            merged,
    ...
    data.update(
        {
            "message": "月度事件统计表调度配置已更新并立即生效" if restart_running else "月度事件统计表调度配置已保存",
