...
def test_download_task_executor_waits_for_late_browser_pool(monkeypatch) -> None:
    fake_pool = _FakePool()
    seen_pages: dict[str, list[object]] = {"A楼": []}
    pool_reads = {"count": 0}

    async def _fake_download_site_with_retry(**kwargs):
        building = str(kwargs["site"]["building"])
        seen_pages.setdefault(building, []).append(kwargs["page"])
        return SimpleNamespace(
            building=building,
            success=True,
            file_path=f"D:\\temp\\{building}.xlsx",
    ...
    monkeypatch.setattr(
        "app.modules.report_pipeline.service.download_task_executor.get_internal_download_browser_pool",
        _fake_get_pool,
    )
    monkeypatch.setattr(
        "app.modules.report_pipeline.service.download_task_executor.async_playwright",
    ...
    pairs = asyncio.run(
        run_download_tasks_by_building(
            ...
            download_tasks=[_Task("A楼", "2026-03-29")],
            feature="月报下载",
            success_stage="内网下载",
            failure_stage="内网下载",
            success_detail_prefix="下载成功 URL=",
            group_download_tasks_by_building=lambda tasks: [("A楼", tasks)],
            download_site_with_retry=_fake_download_site_with_retry,
            log_file_success=lambda **kwargs: success_logs.append(kwargs),
            log_file_failure=lambda **kwargs: failure_logs.append(kwargs),
            browser_pool=None,
    ...
    assert success_logs[0]["building"] == "A楼"
    assert seen_pages["A楼"][0] is fake_pool.pages["A楼"]
