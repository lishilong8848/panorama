...
def test_source_cache_startup_cleanup_deletes_unusable_indexes_only(tmp_path) -> None:
    ...
    store.upsert_source_cache_entry(
        ...
        duty_shift="day",
        downloaded_at="2026-04-21 15:00:00",
        relative_path=valid_file.relative_to(shared_root).as_posix(),
        status="ready",
        size_bytes=2,
    ...
    store.upsert_source_cache_entry(
        ...
        duty_shift="day",
        downloaded_at="2026-04-21 15:00:00",
        relative_path="交接班日志源文件/202604/missing.xlsx",
        status="ready",
        metadata={},
    )

    orphan_index = shared_root / "source_cache_index" / "orphan_family" / "A楼" / "orphan.json"
    orphan_index.parent.mkdir(parents=True, exist_ok=True)
    orphan_index.write_text(
        json.dumps(
            {
                "entry_id": "orphan|latest|x|-|-|A楼",
                "source_family": "orphan_family",
                "building": "A楼",
                "bucket_kind": "latest",
                "bucket_key": "x",
                "relative_path": "orphan.xlsx",
                "status": "ready",
            },
            ensure_ascii=False,
        ),
    ...
    bad_json = shared_root / "source_cache_index" / "bad_family" / "A楼" / "bad.json"
    bad_json.parent.mkdir(parents=True, exist_ok=True)
    bad_json.write_text("{bad", encoding="utf-8")

    result = store.cleanup_source_cache_indexes_once(force=True)

    rows = store.list_source_cache_entries(limit=20)
    assert result["db_deleted_invalid"] == 2
    assert result["index_deleted_invalid_json"] == 1
    assert result["index_deleted_orphan"] == 1
    assert [(row["building"], row["relative_path"]) for row in rows] == [
        ("A楼", valid_file.relative_to(shared_root).as_posix())
    ]
    assert not orphan_index.exists()
    assert not bad_json.exists()
    assert (shared_root / "source_cache_index" / ".startup_cleanup_state.json").exists()
