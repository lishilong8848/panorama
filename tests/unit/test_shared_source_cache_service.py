...
def test_get_latest_ready_selection_requires_index_even_when_shared_files_exist(
    monkeypatch: pytest.MonkeyPatch,
    work_dir: Path,
) -> None:
    shared_root = work_dir / 'shared'
    ...
    monkeypatch.setattr(cache_module, '_now_dt', lambda: cache_module.datetime(2026, 4, 27, 1, 48, 0))
    ...
    assert selection['missing_buildings'] == ['A楼', 'B楼']


def test_external_latest_ready_selection_uses_cached_overview_without_store_scan(
    monkeypatch: pytest.MonkeyPatch,
    work_dir: Path,
) -> None:
    shared_root = work_dir / 'shared'
    ...
    service._external_fast_overview_cache = {  # noqa: SLF001
        'families': [
            {
                'key': FAMILY_HANDOVER_LOG,
                'can_proceed': True,
                'best_bucket_key': '2026-04-27 02',
                'selected_entries': [
                    {
                        'source_family': FAMILY_HANDOVER_LOG,
                        'building': 'A楼',
                        'bucket_kind': 'latest',
                        'bucket_key': '2026-04-27 02',
                        'duty_date': '2026-04-27',
                        'duty_shift': 'day',
                        'downloaded_at': '2026-04-27 02:00:40',
                        'relative_path': '交接班日志源文件/202604/20260427--02/20260427--02--交接班日志源文件--A楼.xlsx',
                        'status': 'ready',
                        'file_path': str(shared_root / '交接班日志源文件' / '202604' / '20260427--02' / '20260427--02--交接班日志源文件--A楼.xlsx'),
                    },
                ],
                'fallback_buildings': [],
                'missing_buildings': [],
    ...
    monkeypatch.setattr(cache_module, '_now_dt', lambda: cache_module.datetime(2026, 4, 27, 2, 20, 0))
    monkeypatch.setattr(
        service,
        '_fast_ready_entries_for_family',
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError('should use cached overview')),
    )

    selection = service.get_latest_ready_selection(source_family=FAMILY_HANDOVER_LOG, buildings=['A楼'])

    assert selection['source'] == 'external_fast_overview_cache'
    assert selection['can_proceed'] is True
    assert selection['selected_entries'][0]['building'] == 'A楼'
    assert selection['selected_entries'][0]['file_path'].endswith('20260427--02--交接班日志源文件--A楼.xlsx')
