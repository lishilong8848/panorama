...
def test_handover_review_capacity_download_returns_file_with_warning_when_sync_not_ready(monkeypatch, tmp_path):
    output_file = tmp_path / "A楼交接班容量报表.xlsx"
    output_file.write_bytes(b"demo")
    container = _FakeContainer()
    ...
    assert any("继续下载业务文件" in message for message in container.logs)
